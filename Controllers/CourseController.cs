...
public class CourseController : Controller
{
    ...
    public async Task<IActionResult> Create(CreateCourseViewModel dtoModel)
    {
        ...
        var createdCourseResult = await _courseService.CreateAsync(dtoModel.Adapt<Models.Course>(), User.Identity?.Name);
    ...
    public async Task<IActionResult> JoinCourse(string key)
    {
        ...
        var existingCurseResult = await _courseService.GetCourseByKeyAsync(key);
        if(!existingCurseResult.IsSuccess)
            return View("NotFound", new CourseMultipleViewModel() { IsInValidKey = true });

        var course = existingCurseResult.Data;
